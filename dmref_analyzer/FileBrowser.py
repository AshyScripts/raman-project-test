...
class FileBrowser(object):
    def __init__(self, root_dir=None, param_rule_file=None):
        if root_dir is None:
            self.root_dir = os.path.expanduser("~/Desktop/project/raman-ashkan/")
        else:
            self.root_dir = root_dir
        if param_rule_file is None:
            param_rule_file = os.path.expanduser(
                "~/Desktop/project/raman-ashkan/parameter_rules.csv"
            )
    ...
    def show_sample_with_experiments(self, sample_rng=np.arange(1, 10000)):
        ...
        df_columns = list(all_experiment_types)
        ...
        for sample_id in sample_rng:
            ...
            entry_series = pd.Series(index=list(all_experiment_types), data=entry)
