...
def skewness(x):
    ...
    return np.max(x) / np.min(x), np.sum(np.power(x_centered, 3)) / (
        (n - 1) * np.power(v, 1.5)
    )
...
def find_file_with_regex(base_dir, file_pattern, sample_id):
    ...
    if len(file_paths) == 1:
        ...
        if file_type not in ["R", "F", "RF"]:
            print(
                "Multiple parameter / data files for sample "
                + str(sample_id)
                + " found: "
            )
...
def gen_normalized_dataset(data, features, outcomes):
    if outcomes:
        ...
        return pd.DataFrame(
            data=scale(data[features].as_matrix(), axis=0), columns=features
        )
